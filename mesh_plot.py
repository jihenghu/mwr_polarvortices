...
if __name__=="__main__":
    ...
    ch=int(sys.argv[1]) ## 0 for channel 1; 5 for channel 6   
    # print(ch)

    for pj in range(51,55):
        if pj==2: continue
        print(f'perijove={pj:02d}; ch={(ch+1):02d}')

        mesh(pj,ch)   
        os.system(f'ncl -Q plot.TA.channels.ncl pj={pj:02d} ch={(ch+1):02d}')

    # os.system(f'convert -delay 100 -loop 0 TA6_PJ??_swath_PolarView.png TA6_PJ_swath_PolarView.gif')
