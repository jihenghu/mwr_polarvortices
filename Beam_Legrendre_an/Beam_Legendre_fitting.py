...
def LoadBeamPattern(Freq):
    global BeamPattern,BeamPattern_raw
    ...
    BeamPattern_raw = 10.0**(BeamPattern/10.0)
...
if __name__=="__main__":
    ...
    for ch in range(0,6):
        ...
        Beam_theta_raw=np.mean(BeamPattern_raw, axis=0)
        ...
        ## the antenna pattern in 1 degree elevation angular resolution
        ax.scatter(mu, Beam_theta_raw, marker='o',s=20,alpha=0.5,c='xkcd:orange',label=f'Raw data')

        ord=50
        ## Legendre expansion
        initial_guess = [1.0] * ord  # Initial guess for coefficients TO 20TH ORDER
        coeffs, covars = curve_fit(func, mu, Beam_theta, p0=initial_guess)

        print(covars)

        sum=np.float64(0)
        nubins=np.linspace(0, 1, 91)
        dmu=nubins[1]-nubins[0]
        for iu in range(0,len(nubins)-1):
            sum=sum+(func(np.float64(nubins[iu]), *coeffs)+func(np.float64(nubins[iu+1]), *coeffs))*0.5*dmu
        
        # the sum of all components should ends up 1/2PI
        ...
        coeffs_norm=np.array(list(map(np.float32,coeffs_norm)))

        coeffs_norm=np.where(coeffs_norm<1E-5,0,coeffs_norm)
        print(coeffs_norm)
        np.savetxt(f"an_legrendre_{ord}ord_ch{ch}.txt", coeffs_norm, fmt="%10.5f")


        # Plot the fitted function
        ax.plot(mu, func(mu, *coeffs), c='xkcd:forest green' , label=f"${ord:02d}^{{th}}$ order Legendre polynomials")
        ax.plot(mu, func(mu, *coeffs_norm), c='red' , label=f"Normalized ${ord:02d}^{{th}}$ order Legendre polynomials")
    ...
    ###
        ...
        fig.savefig(f"Beam_legendre_{ord:02d}th_normalized_ch{ch+1:02d}.png",dpi=300)
